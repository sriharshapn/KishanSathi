...
export const SatellitePage: React.FC<SatellitePageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white rounded-3xl border border-[#CCE0D0] p-6 shadow-xs space-y-4">
          ...
          <div className="pt-2 flex flex-wrap items-center justify-between text-xs text-stone-500 gap-2">
            <span>Powered by Google Maps API & Google Earth Engine (@google/earthengine) • Sentinel-2 MSI 10m GSD</span>
            <button
      ...
      {/* 🏛️ Official Indian Government Cadastre & Plot Registry (AgriStack, ISRO Bhuvan, State RoR) */}
      <div className="bg-white rounded-3xl border border-[#CCE0D0] p-6 shadow-xs space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-[#E2ECE3]">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-emerald-50 text-[#123826] border border-emerald-200">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#EBF5ED] text-[#123826] border border-[#CCE0D0]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-[#F7FBF8] hover:bg-[#EBF5ED] border border-[#CCE0D0] text-[#123826] text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-[#123826] hover:bg-[#1B4D35] text-white text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
            >
        ...
        {/* Survey Query Bar */}
        <form onSubmit={handleSearchGovPlot} className="grid grid-cols-1 md:grid-cols-6 gap-3 p-4 rounded-2xl bg-[#F7FBF8] border border-[#E2ECE3]">
          <div>
            <label className="block text-[11px] font-bold text-stone-600 mb-1">State / RoR</label>
            <input
              type="text"
              readOnly
              value={selectedState}
              className="w-full px-3 py-2 text-xs rounded-xl bg-stone-100 border border-[#CCE0D0] text-stone-700 font-bold focus:outline-none"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-[11px] font-bold text-stone-600 mb-1">Taluk / Tehsil</label>
            <input
              type="text"
              value={talukInput}
              onChange={(e) => setTalukInput(e.target.value)}
              placeholder="e.g. Ballari"
              className="w-full px-3 py-2 text-xs rounded-xl bg-white border border-[#CCE0D0] text-stone-800 font-bold focus:outline-none focus:border-[#2E7D32]"
            />
          </div>
          <div>
            <label className="block text-[11px] font-bold text-stone-600 mb-1">Village</label>
            <input
              type="text"
              value={villageInput}
          ...
          <div>
            <label className="block text-[11px] font-bold text-stone-600 mb-1">Survey No. / Hissa</label>
            <input
              type="text"
              value={surveyInput}
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full py-2 px-3 rounded-xl bg-[#2E7D32] hover:bg-[#1B4D35] text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs disabled:opacity-60"
            >
              {govPlotLoading ? (
                <>
        ...
        {govPlotError && (
          <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-medium flex items-center justify-between">
            ...
            <button onClick={() => setGovPlotError(null)} className="font-bold underline cursor-pointer text-xs">Dismiss</button>
          </div>
        )}

        {/* Government Cadastral Record Result Card */}
        {govPlotResult && (
          <div className="bg-[#FAFDF9] border border-emerald-300 rounded-2xl p-5 space-y-4 shadow-xs animate-in fade-in">
            <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-emerald-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="font-black text-sm text-[#123826]">
                      {govPlotResult.official_plot_id.system_name}
                    </h4>
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-700 text-white">
                      Verified Cadastre
                    </span>
                  </div>
                  <p className="text-[11px] text-stone-500">
                    AgriStack ID: <span className="font-mono font-bold text-[#123826]">{govPlotResult.official_plot_id.agristack_plot_id}</span> • Survey #{govPlotResult.official_plot_id.survey_number}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-[#123826] hover:bg-[#1B4D35] text-white text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shadow-xs"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-3 py-1.5 rounded-xl bg-white hover:bg-stone-50 border border-[#CCE0D0] text-[#123826] text-xs font-bold transition-all flex items-center gap-1"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="p-3 rounded-xl bg-white border border-[#E2ECE3] space-y-1">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Patta / Khata Holder</span>
                <span className="text-xs font-extrabold text-[#123826] block">{govPlotResult.tenure.owner_name}</span>
                <span className="text-[11px] text-stone-500 block">Khata #{govPlotResult.tenure.khata_number} • {govPlotResult.tenure.land_classification}</span>
                <span className="text-[10px] text-emerald-700 font-bold block">Status: {govPlotResult.tenure.tenure_type}</span>
              </div>

              {/* Geospatial Extent */}
              <div className="p-3 rounded-xl bg-white border border-[#E2ECE3] space-y-1">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Cadastral Boundary Extent</span>
                <span className="text-xs font-extrabold text-[#123826] font-mono block">
              ...
              <div className="p-3 rounded-xl bg-white border border-[#E2ECE3] space-y-1">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Digital Crop Survey (DCS)</span>
                <span className="text-xs font-extrabold text-[#123826] block">
              ...
              <div className="p-3 rounded-xl bg-white border border-[#E2ECE3] space-y-1">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Soil Health Card Portal</span>
                <span className="text-xs font-extrabold text-[#123826] block">
                  {govPlotResult.soil_health_card.soil_type} (pH {govPlotResult.soil_health_card.ph})
                </span>
                <span className="text-[11px] text-stone-500 block">
                  N: {govPlotResult.soil_health_card.nitrogen_kg_ha} kg/ha • P: {govPlotResult.soil_health_card.phosphorus_kg_ha} • K: {govPlotResult.soil_health_card.potassium_kg_ha}
                </span>
                <span className="text-[10px] text-stone-600 block">
            ...
            <div className="pt-2 flex flex-wrap items-center justify-between text-xs text-stone-500 border-t border-emerald-100 gap-2">
      ...
      {showGovApisModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl border border-[#CCE0D0] shadow-2xl max-w-3xl w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-[#E2ECE3]">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-emerald-50 text-[#123826] border border-emerald-200">
                  <Landmark className="w-5 h-5 text-[#2E7D32]" />
                ...
                <div>
                  <h3 className="text-base font-extrabold text-[#123826]">
            ...
            <div className="space-y-4">
              {(govApisList.length > 0 ? govApisList : [
                ...
                <div key={api.id} className="p-4 rounded-2xl bg-[#F7FBF8] border border-[#CCE0D0] space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="h-2 w-2 rounded-full bg-emerald-600"></span>
                      <h4 className="font-extrabold text-xs text-[#123826]">{api.system_name}</h4>
                    </div>
                    <a
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px] pt-2 border-t border-[#E2ECE3]">
                    <div>
                      <span className="text-stone-400 block text-[10px]">API Endpoint / Standard</span>
                      <span className="font-mono text-stone-700 break-all">{api.api_endpoint}</span>
                    </div>
                    <div>
                      <span className="text-stone-400 block text-[10px]">Authentication / Protocol</span>
                      <span className="font-mono text-emerald-800 font-bold">{api.auth_type}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="pt-3 flex justify-end border-t border-[#E2ECE3]">
              <button
                type="button"
                onClick={() => setShowGovApisModal(false)}
                className="px-4 py-2 rounded-xl bg-[#123826] text-white text-xs font-bold hover:bg-[#1B4D35] cursor-pointer"
              >
                Close Directory
              </button>
            </div>
      ...
      {showGeeScriptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl border border-[#CCE0D0] shadow-2xl max-w-4xl w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-[#E2ECE3]">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-emerald-50 text-[#123826] border border-emerald-200">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-[#EBF5ED] text-[#123826] border border-[#CCE0D0]">
                      @google/earthengine v1.7.45
                    </span>
                  </div>
                  <p className="text-xs text-stone-500">
                    Official Google Earth Engine JavaScript script configured for <span className="font-bold">{selectedDistrict}, {selectedState}</span> using Copernicus Sentinel-2 Level-2A (COPERNICUS/S2_SR_HARMONIZED).
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="p-3 rounded-xl bg-[#F7FBF8] border border-[#E2ECE3]">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Dataset</span>
                <span className="text-xs font-mono font-bold text-[#123826]">COPERNICUS/S2_SR_HARMONIZED</span>
              </div>
              <div className="p-3 rounded-xl bg-[#F7FBF8] border border-[#E2ECE3]">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Spatial Resolution</span>
                <span className="text-xs font-mono font-bold text-[#123826]">10m GSD (B4 Red & B8 NIR)</span>
              </div>
              <div className="p-3 rounded-xl bg-[#F7FBF8] border border-[#E2ECE3]">
                <span className="text-[10px] uppercase font-bold text-stone-400 block">Official GEE Repo</span>
                <a 
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-stone-700">Executable GEE Code (Ready to Run)</span>
                <button
                  onClick={() => {
              ...
              <pre className="p-4 rounded-2xl bg-stone-900 text-emerald-300 font-mono text-[11px] overflow-x-auto max-h-72 border border-stone-800 select-all leading-relaxed">
            ...
            <div className="pt-3 flex flex-wrap items-center justify-between border-t border-[#E2ECE3] gap-2">
              ...
              <div className="flex items-center gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2 rounded-xl bg-[#2E7D32] hover:bg-[#1B4D35] text-white text-xs font-bold flex items-center gap-1.5 transition-colors shadow-xs"
                >
                ...
                <button
                  type="button"
                  onClick={() => setShowGeeScriptModal(false)}
                  className="px-4 py-2 rounded-xl border border-[#CCE0D0] text-stone-700 text-xs font-bold hover:bg-stone-50 cursor-pointer"
                >
                  Close
                </button>
              </div>
